...
- webpack打包图片
  - 回顾图片引用方式
    ...
    3. 页面<img src="" alt="">引入
       - 此时打包路径会存在问题
       - 所以安装 npm install html-withimg-loader -D

- 现在我需要build出的资源能单独成一个目录
  - 图片资源：在wcj中url-loader中配置路径<br>
    ```js
     use:{
        loader:'url-loader',
        options:{
    ...
    ```
  - css文件：在wcj中MiniCssExtractPlugin配置<br>
    ```js
    new MiniCssExtractPlugin({
    -    filename:'main.css', // 输出css文件名 
